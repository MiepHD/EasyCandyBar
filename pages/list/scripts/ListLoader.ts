					...
					for (const entry of entries) {
						if (entry.isIntersecting) {
							for (let i = 0; i < this.rowlength * 5; i++) {
								...
								$(
									`<a href="../icon/icon.html?icon=${drawable}&type=${type}"><img src="../../projects/${id}/${type}/${drawable}.png"></a>`
								).insertBefore($$("#continuous"));
								data.pop();
	...
	private loadProject(): void {
		RendererAPI.getProjectInfo((info: ProjectStructure) => {
			$$("#open").addEventListener("click", () => {
				RendererAPI.openFolder(this.params.get("type"));
			});
			$$("#import").addEventListener("click", () => {
				RendererAPI.importRequest(() => {
					location.reload();
				});
			});
