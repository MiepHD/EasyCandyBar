...
export class PathChooser {
	...
	private async template(type: string, title: string, ext: string): Promise<string> {
		return this.dialog
			.showOpenDialog(this.BrowserWindow.getFocusedWindow(), {
				properties: [type],
				filters: [
					{
						name: title,
	 ...
	 */
	public async dir(): Promise<string> {
		return this.template("openDirectory", "Project or other icon pack folder", "*");
	}
	...
	public async image(): Promise<string> {
		return this.template("openFile", "Images", "png");
	}
	...
	public async zip(): Promise<string> {
		return this.template("openFile", "Icon Request", "zip");
	}
}
