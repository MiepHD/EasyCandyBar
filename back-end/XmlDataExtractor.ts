...
export class XmlDataExtractor {
	...
	public ByProjectPath(path: string): any {
		...
		for (const file of files) {
			...
			if (!files.includes("drawable") && file.includes("appfilter"))
				this.parseDrawable(this.commentAndObjectTogether(xml));
	...
	private parseAppfilter(other: string): Array<string> {
		...
		if (other.includes("/") && other.includes("{") && other.includes("}")) {
			...
			(packnactiv[0] = splitother[0].split("{")[1]), (packnactiv[1] = splitother[1].split("}")[0]);
		} else {
