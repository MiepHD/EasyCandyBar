...
const RendererAPI = {
	...
	getIcons: (type: string | undefined | null, callback: Functions["icons"]): void => {
		if (!type) return;
		ipcRenderer.on("Icons", (e: any, icons: Array<string>) => {
			callback(icons);
		});
	...
	getIcon: (id: string | undefined | null, callback: Functions["icon"]): void => {
		if (!id) return;
		ipcRenderer.on("Icon", (e: any, icon: Icon) => {
			callback(icon);
		});
	...
	getProjectInfo: (callback: Functions["projectInfo"]): void => {
		ipcRenderer.on("ProjectInfo", (e: any, project: ProjectStructure) => {
			callback(project);
		});
	...
	GET: (path: string | undefined | null, callback: Functions["GETResponse"]): void => {
		if (!path) return;
		ipcRenderer.on("GETResponse", (e: any, file: string) => {
			callback(file);
		});
